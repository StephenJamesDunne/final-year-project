...
export class DQNAgent {
  ...
  async train(): Promise<TrainingStats | null> {
    ...
    if (this.trainingSteps % this.config.targetUpdateFreq === 0) {
      ...
      console.log(`[DQNAgent] Synced target network at step ${this.trainingSteps}`);
    }
  ...
  // Save the agent's state to local browser storage
  async save(name: string = 'five-realms-dqn-agent'): Promise<void> {
    try {
      ...
      const state = {
        ...
        wins: this.wins,
        games: this.games,
        episodeRewards: this.episodeRewards.slice(-20), // Last 20 only
        config: this.config,
  ...
  async load(name: string = 'five-realms-dqn-agent'): Promise<boolean> {
    try {
      ...
      // Load training state
      const stateJson = localStorage.getItem(`${name}-state`);
      if (stateJson) {
