...
export class ExperienceReplay {
  ...
  constructor(maxSize: number = 50000) {
    ...
    console.log(
      `[ExperienceReplay] Created buffer with capacity: ${maxSize.toLocaleString()}`,
    );
  }
  ...
  save(
    key: string = "five-realms-replay-buffer",
    maxToSave: number = 5000,
  ): void {
    try {
      ...
      const savePath = path.resolve(`./models/${key}.json`);
  ...
  load(key: string = "five-realms-replay-buffer"): boolean {
    try {
      const loadPath = path.resolve(`./models/${key}.json`);
      ...
      console.log(
        `[ExperienceReplay] Loaded ${this.buffer.length} experiences from ${loadPath}`,
      );
