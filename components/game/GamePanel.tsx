...
export function GamePanel() {
  ...
  const lastTurnActions = useBattleStore((state) => state.lastTurnActions);
        ...
        >
          {activeTab === "log" ? (
            ...
            <DebugContent
              ...
              lastTurnActions={lastTurnActions}
            />
          )}
        </div>
      </div>
      ...
      {recent.map((entry, i) => (
        <p
          ...
          style={{
            ...
            lineHeight: "1.5",
            color:
              entry.includes("VICTORY") || entry.includes("DEFEAT")
                ? "#fbbf24"
                : "#e2e8f0",
            fontWeight:
              entry.startsWith("═") || entry.startsWith("─") ? 700 : 400,
            marginBottom: "4px",
          }}
        >
    ...
    >
      ...
      <div>
        ...
        {lastTurnActions.length === 0 && (
          <p style={{ fontSize: "10px", color: "#64748b" }}>No actions yet</p>
        )}
        {lastTurnActions.map((action, i) => (
          <p
            key={i}
            style={{
              fontSize: "10px",
  ...
  return (
    ...
    >
      ...
      {isMinion && health !== undefined && (
        <Badge value={health} color="#16a34a" bottom={-6} right={-6} />
      )}
    </div>
  );
}
...
function CardTooltip({ card, x, y }: CardTooltipProps) {
  ...
  const wouldOverflowRight =
    x + TOOLTIP_OFFSET + TOOLTIP_W > window.innerWidth - TOOLTIP_PADDING;
